...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParPays() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f Join f.pays i WHERE i.nom='France'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParListePaysEtAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.film f Join f.pays p WHERE f.annee=2017 AND p.nom", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r Join r.film m JOIN m.realisateurs n WHERE m.annee BETWEEN 2010 AND 2020 AND n.identite='Ridley Scott'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
